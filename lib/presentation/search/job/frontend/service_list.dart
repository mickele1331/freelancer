...
class JobSearchResult extends StatefulWidget {
  const JobSearchResult({super.key, required this.searchKeyword});
...
class _JobSearchResultState extends State<JobSearchResult> {
  ...
  Future<List<JobDetails>?> getFutureMethod(String id) async {
    jobInfoList = await GetJobRemoteServices().getSearchJobList(
      id: id,
  ...
  void initState() {
    setState(() {
      getFutureMethodVariable = getFutureMethod(
        widget.searchKeyword.text.trim(),
      );
    });
